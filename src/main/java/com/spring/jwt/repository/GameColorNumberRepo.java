...
public interface GameColorNumberRepo extends JpaRepository<GameColorNumber, Integer> {
    ...
    @Query(value = "SELECT SUM(g.amount) as totalAmount FROM GameColorNumber g WHERE g.green = :status",nativeQuery = false)
    Integer findByGreen(@Param("status") boolean status);

    @Query(value = "SELECT SUM(g.amount) as totalAmount FROM GameColorNumber g WHERE g.orange = :status",nativeQuery = false)
    Integer findByOrange(@Param("status") boolean status);

    @Query(value = "SELECT SUM(g.amount) as totalAmount FROM GameColorNumber g WHERE g.violet = :status",nativeQuery = false)
    Integer findByViolet(@Param("status") boolean status);
